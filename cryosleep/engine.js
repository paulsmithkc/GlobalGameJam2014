...
Engine.prototype.supportsLocalStorage = function()
{
    try {
...
Engine.prototype.loadData = function()
{
    if (!this.supportsLocalStorage()) return false;
    try
    {
        var storage = window.localStorage;
        var storedData = JSON.parse(storage.getItem(this.storageKey));
        if (storedData === null || storedData.version != this.storageVersion) { return false; }
...
Engine.prototype.saveData = function()
{
    ...
    try
    {
        ...
        storage.setItem(this.storageKey, JSON.stringify(storedData));
        return true;
    }
    catch (e)
    {
...
Engine.prototype.setQuestions = function(questions, options) {
    ...
    this.options.appendChild(questionsDiv);
    this.options.appendChild(optionsDiv);
	for (var i = 0; i < questions.length; i++) {
		...
		textField.type = "text";
		textField.value = "";
		questionsDiv.appendChild(textField);
	}
    for (var i = 0; i < options.length; i++) {
		var button = document.createElement("input");
		button.name = "option";
